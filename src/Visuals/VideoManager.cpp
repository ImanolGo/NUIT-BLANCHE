...
{
    m_fbo.allocate(m_videoPlayer.getPixelsRef().getWidth(), m_videoPlayer.getPixelsRef().getHeight(),GL_RGBA);
...
void VideoManager::update()
{
    ...
    if(m_recordVideoLoops && (m_videoPlayer.getTotalNumFrames() == m_videoPlayer.getCurrentFrame())){
        m_playVideo = false;
...
void VideoManager::onNextVideoChange()
{
   ...
   // m_videoPlayer.close();
    ...
    ofLogNotice()<< "VideoManager::onNextVideoChange-> Loaded Movie: " << m_currentVideo;
    ofLogNotice()<< "VideoManager::onNextVideoChange-> Number Frames: " << m_videoPlayer.getTotalNumFrames();
